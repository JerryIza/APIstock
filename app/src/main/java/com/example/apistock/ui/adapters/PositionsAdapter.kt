...
import com.example.apistock.databinding.PositionsListBinding
...
class PositionsAdapter(
    ...
    fun setItems(inputs: MutableCollection<Pair<Positions, SymbolDetails>>) {
        this.inputs.clear()
...
class PositionViewHolder(private val itemBinding: PositionsListBinding) : RecyclerView.ViewHolder(
    ...
    fun bind(result: Pair<Positions, SymbolDetails>, pos: Int, listener: (Int) -> Unit) {
        ...
        if (positionData.shortQuantity != 0) {
            ...
            itemBinding.pLOpen.text = ("P/L Open: "+("$%,.2f".format((positionData.averagePrice - symbolDetails.lastPrice) *positionData.shortQuantity)))
        } else {
        ...
        when {
            regex.containsMatchIn(symbolDetails.regularMarketNetChange.toString()) -> {
                itemBinding.pChg.setTextColor(ContextCompat.getColor(this.itemView.context, R.color.colorDown))

            }
            symbolDetails.regularMarketNetChange != 0.0 -> {
                itemBinding.pChg.setTextColor(ContextCompat.getColor(this.itemView.context, R.color.colorUp))
